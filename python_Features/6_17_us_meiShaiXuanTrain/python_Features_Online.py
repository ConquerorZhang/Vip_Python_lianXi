...
def Online_Features_i_days(i):
    ...
    ########################################### u-s 特征 ############################################
    #us出现天数（us）
    group = Online.groupby('u_spu_id')
    Online_us_chuXianTianShu = pd.DataFrame(group.size(),columns=['us_chuXianTianShu_'+str(i)]).reset_index()
    #us的点击天数(us)
    ...
    #############################把7天的特征先merge了存下################################################
    # u-s 特征 
    Online_Features_XiaDan = pd.merge(Online_Features_XiaDan,Online_us_chuXianTianShu,how = 'left',on='u_spu_id')
    ...
    #########################################  交叉比值特征  ################################################################
    #us点击量/u点击量
    Online_us_bi_u_dianJiLiang = Online_Features_XiaDan['us_dianJiTianShu_'+str(i)]/Online_Features_XiaDan['u_dianJiLiang_'+str(i)]
    Online_us_bi_u_dianJiLiang = pd.DataFrame(Online_us_bi_u_dianJiLiang, columns=['us_bi_u_dianJiLiang_'+str(i)])
    Online_Features_XiaDan = pd.concat([Online_Features_XiaDan,Online_us_bi_u_dianJiLiang['us_bi_u_dianJiLiang_'+str(i)]],axis=1)
    del Online_us_bi_u_dianJiLiang
    
    #us下单量/u下单量
    Online_us_bi_u_XiaDanLiang = Online_Features_XiaDan['us_xiaDanTianShu_'+str(i)]/Online_Features_XiaDan['u_xiaDanLiang_'+str(i)]
    Online_us_bi_u_XiaDanLiang = pd.DataFrame(Online_us_bi_u_XiaDanLiang, columns=['us_bi_u_XiaDanLiang_'+str(i)])
    Online_Features_XiaDan = pd.concat([Online_Features_XiaDan,Online_us_bi_u_XiaDanLiang['us_bi_u_XiaDanLiang_'+str(i)]],axis=1)
    del Online_us_bi_u_XiaDanLiang
    
    #u-cate点击量/u点击量
    Online_uc_bi_u_dianJiLiang = Online_Features_XiaDan['u_cate_dianJiLiang_'+str(i)]/Online_Features_XiaDan['u_dianJiLiang_'+str(i)]
    Online_uc_bi_u_dianJiLiang = pd.DataFrame(Online_uc_bi_u_dianJiLiang, columns=['uc_bi_u_dianJiLiang_'+str(i)])
    Online_Features_XiaDan = pd.concat([Online_Features_XiaDan,Online_uc_bi_u_dianJiLiang['uc_bi_u_dianJiLiang_'+str(i)]],axis=1)
    del Online_uc_bi_u_dianJiLiang
    
    #u-cate下单量/u下单量
    Online_uc_bi_u_XiaDanLiang = Online_Features_XiaDan['u_cate_xiaDanLiang_'+str(i)]/Online_Features_XiaDan['u_xiaDanLiang_'+str(i)]
    Online_uc_bi_u_XiaDanLiang = pd.DataFrame(Online_uc_bi_u_XiaDanLiang, columns=['uc_bi_u_XiaDanLiang_'+str(i)])
    Online_Features_XiaDan = pd.concat([Online_Features_XiaDan,Online_uc_bi_u_XiaDanLiang['uc_bi_u_XiaDanLiang_'+str(i)]],axis=1)
    del Online_uc_bi_u_XiaDanLiang
    
    #u-brand点击量/u点击量
    Online_ub_bi_u_dianJiLiang = Online_Features_XiaDan['u_brand_dianJiLiang_'+str(i)]/Online_Features_XiaDan['u_dianJiLiang_'+str(i)]
    Online_ub_bi_u_dianJiLiang = pd.DataFrame(Online_ub_bi_u_dianJiLiang, columns=['ub_bi_u_dianJiLiang_'+str(i)])
    Online_Features_XiaDan = pd.concat([Online_Features_XiaDan,Online_ub_bi_u_dianJiLiang['ub_bi_u_dianJiLiang_'+str(i)]],axis=1)
    del Online_ub_bi_u_dianJiLiang
    
    #u-brand下单量/u下单量
    Online_ub_bi_u_XiaDanLiang = Online_Features_XiaDan['u_brand_xiaDanLiang_'+str(i)]/Online_Features_XiaDan['u_xiaDanLiang_'+str(i)]
    Online_ub_bi_u_XiaDanLiang = pd.DataFrame(Online_ub_bi_u_XiaDanLiang, columns=['ub_bi_u_XiaDanLiang_'+str(i)])
    Online_Features_XiaDan = pd.concat([Online_Features_XiaDan,Online_ub_bi_u_XiaDanLiang['ub_bi_u_XiaDanLiang_'+str(i)]],axis=1)
    del Online_ub_bi_u_XiaDanLiang
    ##################################补缺失值  或者最后统一补 #################################
    Online_Features_XiaDan = Online_Features_XiaDan.fillna(value=0)
    #前3列没用了
    x_columns = [x for x in Online_Features_XiaDan.columns if x not in ['u_spu_id', 'uid', 'spu_id']]
    Online_Features_XiaDan = Online_Features_XiaDan[x_columns]
    
    #导出数据
    Online_Features_XiaDan.to_csv('F:/Data_Vip/data/python_Features/Online_Features.csv', index=False)  # 不带索引