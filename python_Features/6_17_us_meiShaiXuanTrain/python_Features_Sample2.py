...
def Sample2_Features_i_days(i):
    ...
    if i==3:
        ...
        Sample2 = Sample2[Sample2.datas < '03-18']
    if i==7:
    ...
    ########################################### u-s 特征 ############################################
    #us出现天数（us）
    group = Sample2.groupby('u_spu_id')
    Sample2_us_chuXianTianShu = pd.DataFrame(group.size(),columns=['us_chuXianTianShu_'+str(i)]).reset_index()
    #us的点击天数(us)
    ...
    #############################把7天的特征先merge了存下################################################
    # u-s 特征 
    Sample2_Features_XiaDan = pd.merge(Sample2_Features_XiaDan,Sample2_us_chuXianTianShu,how = 'left',on='u_spu_id')
    ...
    #########################################  交叉比值特征  ################################################################
    #us点击量/u点击量
    Sample2_us_bi_u_dianJiLiang = Sample2_Features_XiaDan['us_dianJiTianShu_'+str(i)]/Sample2_Features_XiaDan['u_dianJiLiang_'+str(i)]
    Sample2_us_bi_u_dianJiLiang = pd.DataFrame(Sample2_us_bi_u_dianJiLiang, columns=['us_bi_u_dianJiLiang_'+str(i)])
    Sample2_Features_XiaDan = pd.concat([Sample2_Features_XiaDan,Sample2_us_bi_u_dianJiLiang['us_bi_u_dianJiLiang_'+str(i)]],axis=1)
    del Sample2_us_bi_u_dianJiLiang
    
    #us下单量/u下单量
    Sample2_us_bi_u_XiaDanLiang = Sample2_Features_XiaDan['us_xiaDanTianShu_'+str(i)]/Sample2_Features_XiaDan['u_xiaDanLiang_'+str(i)]
    Sample2_us_bi_u_XiaDanLiang = pd.DataFrame(Sample2_us_bi_u_XiaDanLiang, columns=['us_bi_u_XiaDanLiang_'+str(i)])
    Sample2_Features_XiaDan = pd.concat([Sample2_Features_XiaDan,Sample2_us_bi_u_XiaDanLiang['us_bi_u_XiaDanLiang_'+str(i)]],axis=1)
    del Sample2_us_bi_u_XiaDanLiang
    
    #u-cate点击量/u点击量
    Sample2_uc_bi_u_dianJiLiang = Sample2_Features_XiaDan['u_cate_dianJiLiang_'+str(i)]/Sample2_Features_XiaDan['u_dianJiLiang_'+str(i)]
    Sample2_uc_bi_u_dianJiLiang = pd.DataFrame(Sample2_uc_bi_u_dianJiLiang, columns=['uc_bi_u_dianJiLiang_'+str(i)])
    Sample2_Features_XiaDan = pd.concat([Sample2_Features_XiaDan,Sample2_uc_bi_u_dianJiLiang['uc_bi_u_dianJiLiang_'+str(i)]],axis=1)
    del Sample2_uc_bi_u_dianJiLiang
    
    #u-cate下单量/u下单量
    Sample2_uc_bi_u_XiaDanLiang = Sample2_Features_XiaDan['u_cate_xiaDanLiang_'+str(i)]/Sample2_Features_XiaDan['u_xiaDanLiang_'+str(i)]
    Sample2_uc_bi_u_XiaDanLiang = pd.DataFrame(Sample2_uc_bi_u_XiaDanLiang, columns=['uc_bi_u_XiaDanLiang_'+str(i)])
    Sample2_Features_XiaDan = pd.concat([Sample2_Features_XiaDan,Sample2_uc_bi_u_XiaDanLiang['uc_bi_u_XiaDanLiang_'+str(i)]],axis=1)
    del Sample2_uc_bi_u_XiaDanLiang
    
    #u-brand点击量/u点击量
    Sample2_ub_bi_u_dianJiLiang = Sample2_Features_XiaDan['u_brand_dianJiLiang_'+str(i)]/Sample2_Features_XiaDan['u_dianJiLiang_'+str(i)]
    Sample2_ub_bi_u_dianJiLiang = pd.DataFrame(Sample2_ub_bi_u_dianJiLiang, columns=['ub_bi_u_dianJiLiang_'+str(i)])
    Sample2_Features_XiaDan = pd.concat([Sample2_Features_XiaDan,Sample2_ub_bi_u_dianJiLiang['ub_bi_u_dianJiLiang_'+str(i)]],axis=1)
    del Sample2_ub_bi_u_dianJiLiang
    
    #u-brand下单量/u下单量
    Sample2_ub_bi_u_XiaDanLiang = Sample2_Features_XiaDan['u_brand_xiaDanLiang_'+str(i)]/Sample2_Features_XiaDan['u_xiaDanLiang_'+str(i)]
    Sample2_ub_bi_u_XiaDanLiang = pd.DataFrame(Sample2_ub_bi_u_XiaDanLiang, columns=['ub_bi_u_XiaDanLiang_'+str(i)])
    Sample2_Features_XiaDan = pd.concat([Sample2_Features_XiaDan,Sample2_ub_bi_u_XiaDanLiang['ub_bi_u_XiaDanLiang_'+str(i)]],axis=1)
    del Sample2_ub_bi_u_XiaDanLiang
    ##################################补缺失值  或者最后统一补 #################################
    Sample2_Features_XiaDan = Sample2_Features_XiaDan.fillna(value=0)
    #前3列没用了
    x_columns = [x for x in Sample2_Features_XiaDan.columns if x not in ['u_spu_id', 'uid', 'spu_id']]
    Sample2_Features_XiaDan = Sample2_Features_XiaDan[x_columns]
    
    #导出数据  把后边的额天数标记去掉了，不用合并了，周期合并的数量太大了
    Sample2_Features_XiaDan.to_csv('F:/Data_Vip/data/python_Features/Sample2_Features.csv', index=False)  # 不带索引